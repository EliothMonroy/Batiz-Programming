 ...
 * @author ELITH
 */
@WebServlet(name = "sBorrarBD", urlPatterns = {"/BorrarBD"})
public class sBorrarBD extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(sesion_usuario.getAttribute("id_usuario")!=null && sesion_usuario.getAttribute("tipo")!="4" && request.getParameter("Borrar").equals("Borrar"))
            {
                cDatos BD= new cDatos();
                    try{
                        BD.Conexion();
                        ResultSet r=BD.Consulta("call sp_BajaUsuario("+sesion_usuario.getAttribute("id_usuario")+",'"+CURP+"');");
                        if(r.next()){
                            if(r.getInt("valido")==1){
                                response.sendRedirect("sRedirect?redirect=4&msj="+r.getString("msj"));
                            }else{
                                response.sendRedirect("sRedirect?redirect=5&msj="+r.getString("msj"));
                            }                            
                        }else{
